...
} from './service/VerificationTokenService'
...
export default function setup(app: Container) {
    ...
    app.bind<IVerificationTokenService>('IVerificationTokenService').to(
    ...
    app.bind<typeof VerificationToken>('VerificationToken').toConstantValue(
        VerificationToken
    )
    app.bind<typeof Classification>('Classification').toConstantValue(
        Classification
    )
    app.bind<typeof Session>('Session').toConstantValue(Session)
    app.bind<typeof Account>('Account').toConstantValue(Account)
    app.bind<Container>('Container').toDynamicValue(
        (ctx) => ctx.container as Container
    )
}
